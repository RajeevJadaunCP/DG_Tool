...
namespace AirtelPro.WinForms.OutputFile
{
    public partial class OFProcessing : Form
    {
        ...
        public OFProcessing()
        {
            ...
            logString.Append($"\n********************************* Data Processing Tool [{DateTime.Now}] USERNAME:{NewLogin.username} SYSTEM NAME : {Environment.MachineName} *************************************\n");
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    foreach (string fileName in fileNames)
                    {
                        ...
                        if (!IsDuplicateFile(fileName))
                        {
                            ...
                            {
                                MessageBox.Show($"{fileName} duplicate records found: ",
                                                            "Message",
                                                            MessageBoxButtons.OK,
                                                            MessageBoxIcon.Information
                                                            );
                                //logString.Append("\r\n" + DateTime.Now + $"**{Path.GetFileName(fileName)} file record already exist in database.**");
                                logString.Append($"       - {count} duplicate ICCIDs found.\n");
                        ...
                        {
                            MessageBox.Show($"{fileName} already exist: ",
                                                        "Message",
                                                        MessageBoxButtons.OK,
                                                        MessageBoxIcon.Information
                                                        );
                            logString.Append($"       - duplicate filename found.\n");
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                if (cbxCustomer.SelectedIndex > 0 && cbxCircle.SelectedIndex > 0 && cbxProfile.SelectedIndex > 0 && !string.IsNullOrEmpty(txtInputfile.Text.Trim()) && !string.IsNullOrEmpty(txtLicence.Text.Trim()))
                {
                    ...
                    int lotid = 0;
                    string[] filenames = txtInputfile.Text.Split(',');
                    using (SqlConnection con = new SqlConnection(connectionString))
                    {
                        con.Open();
                        SqlDataReader reader = null;
                    ...
                    if (lotid > 0)
                    {
                        ...
                        foreach (string filename in filenames)
                        {
                            ...
                            if (lastInsertedId > 0)
                            {
                                ...
                                saveDataGetProcessHDfiles(lastInsertedId, lotid);
                        ...
                        else if(i== filenames.Length)
                        {
                            ...
                            if(File.Exists(txtLicence.Text.TrimEnd()))
                            {
                                ...
                                else
                                {
                                    ...
                                    if (j < filenames.Length)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            logString.Append($"    - Total error during importaion files:  {total_pro_file - total_dup_file - i}\n");
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("insert into [DataTool_log]([Date],[LogMsg],[User_name]) Values (@Date, @LogMsg, @User_name)", con))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@User_name", NewLogin.username);
                        cmd.ExecuteNonQuery();
                    }
                }
            }
        ...
        public int btnImport_Click(int hdid,int lot)
        {
            ...
            try
            {
                ...
                String Query01 = "SELECT * FROM InPutDataTemplate WHERE CustID = " + OFProcessing.customerID + " and ProfileID =" + OFProcessing.ProfileID;
        ...
        SET VarValue = (
            SELECT KeyValue 
            FROM [dbo].[KeyFieldMaster] 
            WHERE KeyType = @varValue AND IsActive = 1 AND CustomerProfileID = @profileID
        ) 
        ...
        private int btnProcessAll_Click()
        {
            ...
            //                        catch
            //                        {
            //                            myData = line.Substring(int.Parse(Pos_From), line.Length - int.Parse(Pos_From));
            //                        }
            //                        if (var_des == "ICCID")
            //                        {
            //                            first_icicid = myData;
            ...
            //                //{
            //                //    case "ICCID_LD":
            //                //        my_data = (Int64.Parse(first_icicid)).ToString();
            //                //        my_data += GetLuhnCheckDigit(my_data);
            //                //        insert_data(var_name.TrimEnd(), var_des.TrimEnd(), my_data.ToString(), var_type.TrimEnd(), filename_2);
            //                //        break; 

            //                //    case "substring":
            //                //        string variable = dv0[16].ToString().Trim();
            //                //        if(variable=="ICCID")
            //                //        {
            //                //            my_data = (Int64.Parse(first_icicid)).ToString().Substring(int.Parse(Pos_From) - 1, int.Parse(len_data));
            //                //            insert_data(var_name.TrimEnd(), var_des.TrimEnd(), my_data.ToString(), var_type.TrimEnd(), filename_2);
            //                //        }
            //                //        else 
            //                //        {
            //                //            my_data = (Int64.Parse(first_icicid)).ToString();
            //                //            my_data += GetLuhnCheckDigit(my_data);
            //                //            my_data = my_data.Substring(int.Parse(Pos_From) - 1, int.Parse(len_data));
            //                //            insert_data(var_name.TrimEnd(), var_des.TrimEnd(), my_data.ToString(), var_type.TrimEnd(), filename_2);
            //                //        }
            ...
            //                //    case "R4_PF":
            //                //        if (var_des.TrimEnd() == "PIN1_PF")
            //                //        {
            //                //            my_data = padding_filler(PIN1); 
            //                //        }
            //                //        else
            //                //        {
            ...
            //                //    case "MSISDN_F":
            //                //        my_data = MSISDN_F(first_msisdn);
            //                //        insert_data(var_name.TrimEnd(), var_des.TrimEnd(), my_data.ToString(), var_type.TrimEnd(), filename_2);
            //                //        break;

            //                //    case "NS":
            //                //        my_data = nibble_swapped(first_msisdn);
            //                //        insert_data(var_name.TrimEnd(), var_des.TrimEnd(), my_data.ToString(), var_type.TrimEnd(), filename_2);
            //                //        break;
            ...
            //                //        string my_data_2 = DESEncrypt(ki_val);

            //                //        insert_data(var_name.TrimEnd(), var_des.TrimEnd(), my_data_2, var_type.TrimEnd(), filename_2);
            //                //        break;

            //                //    case "AES_128":
            //                //        string my_data_opc = OPC_GEN.opc(ki_val, "436F6C6F72504C4153541220184E6F69");
            //                //        insert_data(var_name.TrimEnd(), var_des.TrimEnd(), my_data_opc, var_type.TrimEnd(), filename_2);
            //                //        break;

            //                //    case "AES_128_1":
            //                //        insert_data(var_name.TrimEnd(), var_des.TrimEnd(), Aes_128(Int64.Parse(first_icicid).ToString()), var_type.TrimEnd(), filename_2);
            //                //        break;
            ...
            //                //}
            //                //if (var_des.TrimEnd() == "PIN2")
            //                //{
            //                //    PIN2 = my_data;
            //                //}
            //                //else if (var_des.TrimEnd() == "PUK1")
            //                //{
            //                //    PUK1 = my_data;
            //                //}
            //                //else if (var_des.TrimEnd() == "PUK2")
            //                //{
            //                //    PUK2 = my_data;
            //                //}
            //            }
            ...
            for (int i = 1; i <=records; i++)
            {
                ...
                foreach (DataRow dv0 in dt1.Rows)
                {
                    ...
                    string variableID = dv0[12].ToString().TrimEnd();
                    string lineno = dv0[13].ToString().TrimEnd();
                        ...
                        switch (caseSwitch)
                        {
                            ...
                            case "ACC_Hex":
                                ...
                                else
                                {
                                    ...
                                    {
                                        my_data = acc((Int64.Parse(newRow[variableID].ToString()) + i).ToString());
                                    }
                            ...
                            case "KI_AES_128":
                                if (varCount == 1)
                                {
                                    ...
                                    my_data = triple_Des(newRow[varIDs[0]].ToString(), newRow[varIDs[1]].ToString());
                                }
                                else if(varCount == 0)
                                {
                                    //my_data = Single_Des(newRow[variableID].ToString());
                                    my_data = triple_Des(newRow[variableID].ToString(), "db4389530b991a10b557246db732cd9a");
                            ...
                            case "KI_AES_128_2":
                                ...
                                {
                                    //my_data = Single_Des(newRow[variableID].ToString());
                                    my_data = triple_Des(newRow[variableID].ToString(), "E8F8D8DCAA7DF2D372B0446C196E580C");
        ...
        public string Aes_128(string asciiString)
        {
            string hexString = StringToHex(asciiString);
            string hex_enc = "";
            using (AesManaged aes = new AesManaged())
            {
                byte[] encrypted_1 = Encrypt_1(StrToByteArray(M2_padding(hexString)), StrToByteArray("CE9E4B11C494410FDA8A5F05D0805512"), StrToByteArray("00000000000000000000000000000000"));
                hex_enc = BitConverter.ToString(encrypted_1).Replace("-", "");
            }
            return hex_enc;
        }
                    ...
                    using (SqlCommand cmd = new SqlCommand("SELECT FILENAME FROM CustProfileFile where CustProfileID = @custProfileID and CustomerID=@custID and  FileIOID<>'I'", con))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (reader.Read())
                            {
                                ...
                                //        MessageBox.Show($"Configuration for {fileName} is not present.");
                                //        break;
                                //}
                            }
                        }
        ...
        public void generate_AllTypeOutput(string filetype)
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                using (SqlDataAdapter sda = new SqlDataAdapter($"SELECT FilePath,FileNamingConv,FileMasterID,FileExtn,CustProfileFileID FROM CustProfileFile where CustProfileID={ProfileID} AND CustomerID={customerID} AND FileName='{filetype}'", con))
                {
                    ...
                    Outfilelocation = rootdir + $"\\{cbxCustomer.Text}\\{cbxProfile.Text}\\{FileProcessingLotID}_{lastInsertedId}_{unixTime}";
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                ...
                if(filenameconv.Trim()=="FROMFILE")
                {
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        SqlCommand command = new SqlCommand($"SELECT t2.FilePath FROM [DataTool].[dbo].[CustProfileFile] t1 INNER JOIN [DataGenProcessHDFile] t2 on t1.CustProfileFileID=t2.CustProfileFileID WHERE FileIOID='I' and t2.DataGenProcessHDID={lastInsertedId}", connection);
                        connection.Open();
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                ...
                using (SqlCommand sqlcom2 = new SqlCommand(Query2, con))
                {
                    ...
                    foreach (DataRow dv0 in dt2.Rows)
                    {
                        ...
                        //        {
                        //            con_cps_1.Open();
                        //            SqlCommand com_cps = new SqlCommand("SELECT " + (bar_1) + "  FROM [DataTool].[dbo].[DataGenProcessDataRecord]  where [DataGenProcessDataRecordID] = " + record_no + "  and DataGenProcessHDID =  '" + fileid + "'", con_cps_1);
                        ...
                        while ((str = reading.ReadLine()) != null)
                        {
...

                            {
                                ...
                                {
                                    con_cps.Open();
                                    SqlCommand com_cps = new SqlCommand("SELECT " + (bar) + "  FROM [DataTool].[dbo].[DataGenProcessDataRecord]  where [DataGenProcessDataRecordID] = " + record_no + "  and DataGenProcessHDID =  '" + fileid + "'", con_cps);
        ...
        private void saveDataGetProcessHDfiles(int dataGenProcessHDID,int lot)
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                con.Open();
        ...
        {
            logString.Append($"****************************************************************[Logging Out] Data Processing Tool is closing *********************************************************************\n");
            System.IO.File.AppendAllText(log_dir + "/Logging/" + $"{DateTime.Now.ToString("dd-MM-yyyy")}_log.txt", logString.ToString());
            upload_log();
            logString.Clear();
            this.Close();
