...
namespace DG_Tool.WinForms.OutputFile
{
    public partial class OFProcessing_Multi : Form
    {
        ...
        private void btnInputFile2_Click(object sender, EventArgs e)
        {
            ...
            if (cbxCustomer2.SelectedIndex > 0 && cbxCircle.SelectedIndex > 0 && cbxProfile.SelectedIndex > 0)
            {
                txtInputfile.Text = "";
                InsertedHDIDS.Clear();
                logString.Append($"\n1. User initiated the data processing tool and selected the following input:-\n    Customer 2 : {cbxCustomer2.Text}\n    Circle 2 : {cbxCircle2.Text}\n    Profile 2 : {cbxProfile2.Text}\n");
                Console.WriteLine($"\n1. User initiated the data processing tool and selected the following input:-\n    Customer 2 : {cbxCustomer2.Text}\n    Circle 2 : {cbxCircle2.Text}\n    Profile 2 : {cbxProfile2.Text}\n");
                string filepath = string.Empty;
                OpenFileDialog openFileDialog = new OpenFileDialog();
                openFileDialog.Multiselect = true;
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    string[] fileNames = openFileDialog.FileNames;
                    logString.Append($"\n2. Selected Directory Path 2: [{Path.GetDirectoryName(fileNames[0])}]\n");
                    Console.WriteLine($"\n2. Selected Directory Path 2: [{Path.GetDirectoryName(fileNames[0])}]\n");
                    logString.Append($"\n3. {fileNames.Length} file selected.\n");
                    Console.WriteLine($"\n3. {fileNames.Length} file selected.\n");
                    total_pro_file = fileNames.Length;
        ...
        private void cbxCustomer2_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbxCustomer2.SelectedIndex > 0)
            {
                var circulList = CommonClass.GetCircle(Convert.ToInt32(cbxCustomer2.SelectedValue));
                ...
                if (circulList != null && circulList.Count > 0)
                {
                    ...
                    cbxCircle2.DataSource = circulList;
                    cbxCircle2.DisplayMember = "CircleName";
                    cbxCircle2.ValueMember = "CircleID";

                }
                else
                {
                    cbxCircle2.DataSource = null;
                }
            }
        }
        ...
        private void cbxCircle2_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbxCircle2.SelectedIndex > 0)
            {
                var customerProfile = CommonClass.GetCustomerProfileList(Convert.ToInt32(cbxCustomer.SelectedValue), Convert.ToInt32(cbxCircle2.SelectedValue));
                ...
                if (customerProfile != null && customerProfile.Count > 0)
                {
                    ...
                    cbxProfile2.DataSource = customerProfile;
                    cbxProfile2.DisplayMember = "ProfileName";
                    cbxProfile2.ValueMember = "ProfileID";
                }
                else
                {
                    cbxProfile2.DataSource = null;
                }
            }
                            ...
                            if (File.Exists(licenceFile))
                            {
                                ...
                                else
                                {
                                    ...
                                    else if (j == 0)
                                    {
                                        ...
                                        Console.WriteLine($"    - Total error during importaion files:  {total_pro_file - total_dup_file - i}\n");
                                    }
                                    else
                                    {
                                        using (SqlConnection con = new SqlConnection(connectionString))
            ...
            if (IsIncremental)
            {
                try
                {
                    ...
                    foreach (DataRow dv0 in dt01.Rows)
                    {
                        ...
                        Console.WriteLine($"inseriting all data : " + var_name);
