...
namespace DG_Tool.WinForms.OutputFile
{
    public partial class OFProcessing : Form
    {
        ...
        public static string timestamp = "", date_format = "";
        int fileid = 0;
        int records = 0;
...
ORDER BY f.DataGenProcessHDID,il.Value";
        ...
        //public string iccid_dupcheck(string filename)   ////commneting beacuse if aftel huge data
        //{
        //    logString.Append("       - Checking for duplicate records in the database.\n");
        ...
        //    SELECT [VarName], [PositionFrom], [Len], [LineNumber], [Tag]
        //    FROM [InPutDataTemplate]
        //    WHERE [CustID] = @CustID AND [ProfileID] = @ProfileID AND [VarDes] = @VarDes and vartext = 'FL'  order by VarName", con))
        //        {
        //            cmd.Parameters.AddWithValue("@CustID", cbxCustomer.SelectedValue);
        //            cmd.Parameters.AddWithValue("@ProfileID", cbxProfile.SelectedValue);
        //            cmd.Parameters.AddWithValue("@VarDes", str);
                            ...
                            if (Var_Text.TrimEnd() == "AL")
                            {
                                ...
                                switch (caseSwitch)
                                {
                                    ...
                                    case "Input_Filename":
                                        ...
                                        string fileNameOnly = Path.GetFileName(filename_2);
     ...
     batchtypename.Equals("MFF2", StringComparison.OrdinalIgnoreCase)) ) )
        ...
        //public int generate_AllTypeOutput_26092025(string filetype)
        //{
        //    try
        //    {
        //        string rootdir = "";
        ...
        //                        int numFiles = (lines.Length) / mca_batchsize;
        //                        logString.Append($"    - MCA file starting splitted into {numFiles} parts \n");
        //                        for (int i = 0; i < numFiles; i++)
        //                        {
        ...
        //    row["Start_ICCID"] = NibbleSwap(startArray[0]);
        //    //Console.WriteLine(row);
        //    row["End_ICCID"] = NibbleSwap(endArray[0]);
        //    //Console.WriteLine(NibbleSwap(endArray[1]).Substring(0, NibbleSwap(endArray[1]).Length - 1));
        //    row["Start_IMSI"] = NibbleSwap(startArray[1]).Substring(3, startArray[1].Length - 3);
        //    //Console.WriteLine(NibbleSwap(startArray[0]).Substring(3));
        //    row["End_IMSI"] = NibbleSwap(endArray[1]).Substring(3, endArray[1].Length - 3);
        //    row["BatchNumber"] = batchIndex.ToString().PadLeft(4, '0');
        //    //Console.WriteLine(NibbleSwap(endArray[0]).Substring(3));
        //    dt.Rows.Add(row);
        //}
       ...
       public static DataTable ConvertCsvToDataTable(string csvFilePath)
        ...
        private void saveDataGetProcessHDfiles_new(int dataGenProcessHDID, int lot, string filename_original)
        {
            ExecuteDb<object>(con =>
            {
                using var cmd = new SqlCommand("Select * FROM CustProfileFile WHERE CustomerID = @customerId AND CustProfileID = @customerProfileID", con);
                {
                    cmd.CommandType = CommandType.Text;
                    cmd.Parameters.AddWithValue("@customerId", customerID);
