...
namespace DG_Tool.WinForms.OutputFile
{
    ...
    public partial class OFProcessing : Form
    {
        ...
        public static int customerID = 0, totalFiles_progressbar = 0, currentFile_progressbar = 0;
        ...
        public static string timestamp = "", date_format = "", lastCodeMSN = "", lastCodeMSC = "" , firstCodemsn = "", firstCodemsc = "", OPkeyUsed = "", TKkeyUsed = "", relianceInputFilename = "";
        int fileid = 0;
        int records = 0;
    ...
    = new Dictionary<string, Dictionary<string, (string, int)>>(StringComparer.OrdinalIgnoreCase)
{
    ...
    { "MSISDN", new Dictionary<string, (string, int)>() }
        ...
        public void upload_log()
        {
            try
            {
                ...
                // Create Log_Folder in application directory
                string logFolder = Path.Combine(Application.StartupPath, "Log_Folder");

                if (!Directory.Exists(logFolder))
                    Directory.CreateDirectory(logFolder);

                // File name: DG_Log_yyyyMMdd_HHmmss.csv
                string logFile = Path.Combine(
                    logFolder,
                    $"DG_Log_{DateTime.Now:yyyyMMdd_HHmmss}.csv");

                // Create CSV
                using (StreamWriter sw = new StreamWriter(logFile, false, Encoding.UTF8))
                {
                    // Header
                    sw.WriteLine("Date,UserName,LogMessage");

                    // Escape commas, quotes and new lines
                    string logMessage = logString.ToString()
                        .Replace("\"", "\"\"")
            ...
            {
                MessageBox.Show($"Error while saving log.\n\n" + ex.Message + "\n\nStack Trace:\n" + ex.StackTrace);
            }
        }
        ...
        private void CreateProgressBar()
        {
            RunOnUI(() =>
            {
                gb_progress.Controls.Clear();
                _progressBar = new ProgressBar() { Minimum = 0, Maximum = 100, Value = 0, Width = gb_progress.Width - 20, Height = 20, Left = 10, Top = 20, Anchor = AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Top };
                _progressLabel = new Label() { Text = "0%", AutoSize = true, Left = 10, Top = 45 };
                gb_progress.Controls.Add(_progressBar);
                gb_progress.Controls.Add(_progressLabel);
            });
        }
        private void UpdateProgress(int value, string message = "")
        {
            RunOnUI(() =>
            {
                ...
                _progressLabel.Text = $"{value}% - {message}";
            });
        }
        ...
        {
            var inputFiles = txtInputfile.Text
                .Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries)
                .Select(x => Path.GetFileName(x.Trim()))
                .ToList();

            var licenceFiles = txtLicence.Text
                .Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries)
                .Select(x => Path.GetFileName(x.Trim()))
                .ToList();

            // Check file count
            if (inputFiles.Count != licenceFiles.Count)
            {
                MessageBox.Show(
                    $"File count mismatch.\n\n" +
                    $"Input Files   : {inputFiles.Count}\n" +
                    $"License Files : {licenceFiles.Count}",
                    "Validation",
                    MessageBoxButtons.OK,
            ...
            {
                if (!string.Equals(inputFiles[i], licenceFiles[i], StringComparison.OrdinalIgnoreCase))
                {
                    MessageBox.Show(
                        $"Filename mismatch at position {i + 1}.\n\n" +
                        $"Input   : {inputFiles[i]}\n" +
                        $"License : {licenceFiles[i]}",
                        "Validation",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Error);
                    ...
                    if (Path.GetDirectoryName(fileNames[0]).ToString().ToLower().Contains(cbxCustomer.Text.ToLower()))
                    {
                        ...
                        foreach (string fileName in fileNames)
                        {
                            ...
                            if ((!IsDuplicateFile(fileName)))
                            {
                                ...
                                //MessageBox.Show($"Tool will proceeed  without IMSI duplicity \nAre you ok to proceed", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        public string iccid_dupcheck_new_faster(string filename)
        {
            ...
            //return ""; // Temporarily bypassing the duplicate check for testing purposes
            int iccid_len_1 = 0;
        ...
        public bool IsPreviousFileProcessed(string? dummyInputFileName, SqlConnection conn)
        {
            if (string.IsNullOrEmpty(dummyInputFileName)) return true;

            const string checkQuery = @"SELECT CASE WHEN Count1.Total = Count2.Total THEN 1 ELSE 0 END AS [Output]
                                    FROM 
                                    (
                                        SELECT COUNT(*) AS Total
                                        FROM CustProfileFile cpf
                                        INNER JOIN DataGenProcessHD hd ON cpf.CustProfileID = hd.CustProfileID
                                        INNER JOIN DataGenProcessHDFile hdf ON hd.DataGenProcessHDID = hdf.DataGenProcessHDID
                                        WHERE hdf.FileName = @InputFileName AND cpf.FileIOID <> 'I'
                                    ) AS Count1,
                                    (
                                        SELECT COUNT(*) AS Total
                                        FROM DataGenProcessHDFile hdf2
                                        INNER JOIN DataGenProcessHDFile hdf1 ON hdf2.DataGenProcessHDID = hdf1.DataGenProcessHDID
                                        WHERE hdf1.FileName = @InputFileName AND hdf2.OutFlileStatus = 6
                                    ) AS Count2;";

            using var cmd = new SqlCommand(checkQuery, conn);
            cmd.Parameters.AddWithValue("@InputFileName", dummyInputFileName);

            // Fixed: ExecuteScalar is cleaner here since it returns a single row/col value
            object? result = cmd.ExecuteScalar();
            return result != null && Convert.ToInt32(result) == 1;
        }
        public string dummyfilevalidation(string profileName)
        {
            using var conn = new SqlConnection(connectionString);
            conn.Open();
            int dataid = 0;
            string? generatedIccid = "";
            string? generatedImsi = "";
            string? inputfilename = null;
            const string selectQuery = @"SELECT TOP 1 [ID], LastICCID, LastIMSI , InputFileName FROM DummyProfileTracker WHERE ProfileName = @ProfileName ORDER BY LastProcessedDate DESC";

            using (var selectCmd = new SqlCommand(selectQuery, conn))
            {
                selectCmd.Parameters.AddWithValue("@ProfileName", profileName);
                using var reader = selectCmd.ExecuteReader();
                if (reader.Read())
                {
                    dataid = int.Parse(reader["ID"].ToString()!);
                    generatedIccid = (decimal.Parse(reader["LastICCID"].ToString()!) + 1).ToString("0");
                    generatedImsi = (decimal.Parse(reader["LastIMSI"].ToString()!) + 1).ToString("0");
                    inputfilename = reader["InputFileName"].ToString();
                }
            }
            if (string.IsNullOrWhiteSpace(generatedIccid) || string.IsNullOrWhiteSpace(generatedImsi))
            {
                return "false";
            }

            bool completed = IsPreviousFileProcessed(inputfilename,conn);

            if (completed)
            {
                return (generatedIccid + "," + generatedImsi);
            }

            // Clean up the unprocessed record safely without recursion
            const string deleteQuery = "DELETE FROM DummyProfileTracker WHERE Id = @Id"; // Added Space
            using (var deleteCmd = new SqlCommand(deleteQuery, conn))
            {
                ...
                if (rowsAffected > 0)
                {
                    // Log this or notify UI asynchronously. Avoid blocking inside data logic.
                    Console.WriteLine($"Cleaned up unprocessed tracker entry ID: {dataid}");
                    MessageBox.Show("Records deleted successfully for last unprocessed dummy file.");
                }
            }
            string data_variable = dummyfilevalidation(profileName);
            if (data_variable == "false")
            {
                return "false";
            }
            generatedIccid = data_variable.Split(',')[0];
            generatedImsi = data_variable.Split(',')[1];
        ...
        public string GenerateFile(string firstIccid, string firstImsi, int quantity, string profileName, int profileId)
        {
            using var conn = new SqlConnection(connectionString);
            conn.Open();

            string generatedIccid = firstIccid;
            string generatedImsi = firstImsi;

            string fileContent = "", filepath = "";
            const string selectQuery1 = @"select PF1 from CustProfile where ProfileID = @ProfileID";
            using (var selectCmd = new SqlCommand(selectQuery1, conn))
            {
                selectCmd.Parameters.AddWithValue("@ProfileID", profileId);
                using var reader = selectCmd.ExecuteReader();
                if (reader.Read()) fileContent = reader["PF1"].ToString() ?? "";
            }

            const string selectQuery2 = "select filepath from CustProfileFile where CustProfileID = @ProfileID and FileName = 'INP'";
            using (var selectCmd = new SqlCommand(selectQuery2, conn))
            {
                selectCmd.Parameters.AddWithValue("@ProfileID", profileId);
                using var reader = selectCmd.ExecuteReader();
                if (reader.Read()) filepath = reader["filepath"].ToString() ?? "";
            }
            
            string folderPath = Path.Combine(filepath, "Dummy Input Files");
            Directory.CreateDirectory(folderPath);
            string dummyinputfilename = $"{cbxCustomer.Text}_{cbxProfile.Text}_DUMMY_{DateTime.Now:yyyyMMddHHmmss}.txt";
            string filePath = Path.Combine(folderPath, dummyinputfilename);
            fileContent = fileContent
                .Replace("{generatedImsi}", generatedImsi)
                .Replace("{generatedIccid}", generatedIccid)
                .Replace("{quantity}", quantity.ToString());
            File.WriteAllText(filePath, fileContent);
            decimal startIccid = decimal.Parse(generatedIccid);
            decimal startImsi = decimal.Parse(generatedImsi);
            string lastProcessedIccid = (startIccid + quantity - 1).ToString("0");
            string lastProcessedImsi = (startImsi + quantity - 1).ToString("0");
            const string insertQuery = @"
    INSERT INTO DummyProfileTracker 
    (ProfileName, StartICCID, LastICCID, StartIMSI, LastIMSI, InputFileName, LastQty, LastProcessedDate)     
    VALUES     
    (@ProfileName, @StartICCID, @LastICCID, @StartIMSI, @LastIMSI, @InputFileName, @LastQty, @LastProcessedDate)";

            using (var insertCmd = new SqlCommand(insertQuery, conn))
            {
                insertCmd.Parameters.AddWithValue("@ProfileName", profileName);
                insertCmd.Parameters.AddWithValue("@StartICCID", startIccid);
                insertCmd.Parameters.AddWithValue("@LastICCID", lastProcessedIccid);
                insertCmd.Parameters.AddWithValue("@StartIMSI", startImsi);
                insertCmd.Parameters.AddWithValue("@LastIMSI", lastProcessedImsi);
                insertCmd.Parameters.AddWithValue("@InputFileName", dummyinputfilename);
                insertCmd.Parameters.AddWithValue("@LastQty", quantity);
                insertCmd.Parameters.AddWithValue("@LastProcessedDate", DateTime.Now);
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            date_format = "";
            //MessageBox.Show($"Are you sure you want to process data without duplicate checks?",
            //                                      "Message",
            //                                      MessageBoxButtons.OK,
            //                                      MessageBoxIcon.Information
            //                                      );
            ...
            else if(profilename.Contains("DUMMY"))
            {
                dummy_file_qty = 0;
                string valid = dummyfilevalidation(profilename);



                string START_ICCID = "", START_IMSI = "", QTY = "";
                if (valid != "false")//for true condition
                {
                    QTY = Interaction.InputBox(
                        "Enter Data Quantity (must be greater than 0):",
                        "Data Quantity",
                        "1"); // default value
                    ...
                    START_ICCID = parts[0].Trim();
                    START_IMSI = parts[1].Trim();
                ...
                {
                    START_ICCID = Interaction.InputBox("Enter START_ICCID (must be 19 digits):",
                        "Data Quantity",
                        "1"); // default value

                    START_IMSI = Interaction.InputBox("Enter START_IMSI (must be 15 digits):",
                        "Data Quantity",
                        "1"); // default value

                    QTY = Interaction.InputBox("Enter Data Quantity (must be greater than 0):",
                        "Data Quantity",
                        "1"); // default value

                    if (string.IsNullOrWhiteSpace(QTY)|| string.IsNullOrWhiteSpace(START_ICCID) || string.IsNullOrWhiteSpace(START_IMSI)|| START_IMSI.Length!=15 || START_ICCID.Length!=19 )
                    {
                        // User cancelled
                        return;
                    }
                ...
                txtInputfile.Text = GenerateFile(START_ICCID, START_IMSI, int.Parse(QTY), profilename,ProfileID);
                inputFile = txtInputfile.Text.Trim();
                //return;
            }
            ...
            try
            {
                if (customerID > 0 && circleID > 0 && ProfileID > 0 && !string.IsNullOrEmpty(inputFile) && !string.IsNullOrEmpty(licenceFile))
                {
                    ...
                    if (lotid > 0)
                    {
                        ...
                        //for progress bar
                        totalFiles_progressbar = (filenames.Length * 3) + 5;
                        currentFile_progressbar = 0;
                        int file_count_progress_bar = 0;
                        foreach (string filename in filenames)
                        {
                            ...
                            if (lastInsertedId > 0)
                            {
                                ...
                                //for progressbar
                                currentFile_progressbar++;
                                file_count_progress_bar++;
                                int percentage = (currentFile_progressbar * 100) / totalFiles_progressbar;

                                UpdateProgress(
                                    percentage,
                                    $"Importing files :  File_No ({file_count_progress_bar})"
                                );
                        ...
                        //Initial processing done for first record of each file and moving to next step
                        ...
                        else if (i == filenames.Length)
                        {
                            ...
                            if (licenceFile != "Selected profile has no Licence file")
                            {
                                ...
                                {
                                    MessageBox.Show(status);
                                    logString.Append($"{status}License Configuration error\n");
                                    throw new InvalidOperationException("License Configuration error");
                                }
                            ...
                            int j = 0;
                            //totalFiles_progressbar = (InsertedHDIDS.Count*2)+5;
                            //currentFile_progressbar = 0;
                            file_count_progress_bar = 0;
                            foreach (int hdid in InsertedHDIDS)
                            {
                                //for progressbar
                                currentFile_progressbar++;
                                ...
                                UpdateProgress(
                                    percentage,
                                    $"Processing {hdid} : - File_No ({file_count_progress_bar}/{InsertedHDIDS.Count})"
                                );
                                ...
                                if (test_data_validation == 10)
                                {
                                    logString.Append($"HSM error Connectivity Lost!!!\n");
                                    throw new InvalidOperationException("HSM error Connectivity Lost!!!");
                                }
                                if (test_data_validation == 100)
                                {
                                    logString.Append($"Data Configuration error\n");
                                    throw new InvalidOperationException("Data Configuration error");
                                }
                ...
                if (FileProcessingLotID > 0)
                {
                    ...
                    foreach (var id in ids)
                    {
                        ...
                        if (lastInsertedId > 0)
                        {
                            ...
                            UpdateProgress(
                                percentage,
                                $"Generating file  ({file_count_progress_bar}/{ids.Count})"
                            );
        ...
        public int First_Record_Processing(int hdid, int lot)
        {
            ...
            if (IsIncremental)
            {
                try
                {
                    ...
                    {
                        logString.Append($"Unable to read ICCID from input file Linenumber:{iccid_line_no} , position from : {iccid_frm} , length : {iccid_len}");
                    ...
                    {
                        logString.Append($"Unable to read IMSI from input file Linenumber:{imsi_line_no} , position from : {imsi_from} , length : {imsi_len}");
                        MessageBox.Show($"Unable to read IMSI from input file Linenumber:{imsi_line_no} , position from : {imsi_from} , length : {imsi_len}");
                        throw;
                    ...
                    {
                        logString.Append($"Unable to read MSISDN from input file Linenumber:{msisdn_line_no} , position from : {msisdn_from} , length : {msisdn_len}");
                        MessageBox.Show($"Unable to read MSISDN from input file Linenumber:{msisdn_line_no} , position from : {msisdn_from} , length : {msisdn_len  }");
                        throw;
                    ...
                    for (int i = 0; i < qty; i++)
                    {
                        if (msisdn != 0)
                        {
                            resultDataTable.Rows.Add(StringToHex(iccid.ToString()).Trim(), StringToHex(imsi.ToString()).Trim(), StringToHex(msisdn.ToString()).Trim(), iccid.ToString().PadLeft(iccid_len, '0'), imsi.ToString().PadLeft(imsi_len, '0'), msisdn.ToString().PadLeft(msisdn_len, '0'));
                        ...
                        else
                        {
                            ...
                            resultDataTable.Rows.Add(StringToHex(iccid.ToString()).Trim(), StringToHex(imsi.ToString()).Trim(), "", iccid.ToString().PadLeft(iccid_len, '0'), imsi.ToString().PadLeft(imsi_len, '0'), "");
                            iccid += 1;
                            imsi += 1;
                        }
                            ...
                            if (Var_Text.TrimEnd() == "AL")
                            {
                                ...
                                switch (caseSwitch)
                                {
                                    ...
                                    case "FP":
                                        ...
                                        myData = F_Padding(Process_data.Select($"Variable = '{tag_value}'")[0]["Value"].ToString(), int.Parse(len_data));
                                    ...
                                    case "ICCID_LD_U":
                                        ...
                                        myData += GetLuhnCheckDigit_U(myData);
                                    ...
                                    case "SHA1":
                                        myData = ComputeHash(Process_data.Select($"Variable = '{tag_value}'")[0]["Value"].ToString(), "SHA1");
                                        break;

                                    case "SHA256":
                                        myData = ComputeHash(Process_data.Select($"Variable = '{tag_value}'")[0]["Value"].ToString(), "SHA256");
                                        break;

                                    case "SHA512":
                                        myData = ComputeHash(Process_data.Select($"Variable = '{tag_value}'")[0]["Value"].ToString(), "SHA512");
                                        break;
                                    ...
                                    case "KI_AES_128":
                                        ...
                                        {
                                            logString.Append($"**Error: KI_AES_128 requires 2 variables, but got: {tag_value}**\n");
                                    ...
                                    case "Single_Des":
                                        ...
                                        {
                                            logString.Append($"**Error: Single_Des requires 2 variables, but got: {tag_value}**\n");
                                    ...
                                    case "AES_128":
                                        ...
                                        {
                                            logString.Append($"**Error: AES_128 requires 2 variables, but got: {tag_value}**\n");
                                            throw new Exception($"AES_128 requires 2 variables, but got: {tag_value}");
                                        }
                                    ...
                                    case "AES256_ECB":
                                        ...
                                        {
                                            logString.Append($"**Error: AES256_ECB requires 2 variables, but got: {tag_value}**\n");
                                            throw new Exception($"AES256_ECB requires 2 variables, but got: {tag_value}");
                                        }
                                    ...
                                    case "AES256_CBC":
                                        ...
                                        {
                                            logString.Append($"**Error: AES256_CBC requires 2 variables, but got: {tag_value}**\n");
                                            throw new Exception($"AES256_CBC requires 2 variables, but got: {tag_value}");
                                        }
                                    ...
                                    case "Triple_Des_CBC":
                                        ...
                                        {
                                            logString.Append($"**Error: Triple_Des_CBC requires 2 variables, but got: {tag_value}**\n");
                                    ...
                                    case "euicc_cert_data":
                                        ...
                                        {
                                            logString.Append($"**Error: Hsm connectivity issue {tag_value}**\n");
        ...
        private int All_Record_Processing()
        {
            ...
            if (customer_name_form.ToUpper() == "RELIANCE")
            {
                string query = @"SELECT TOP 1 LastGeneratedCodeMSN, LastGeneratedCodeMSC
                     FROM CodeTracker 
                     WHERE Prefix = @prefix order by id desc";
                ...
                firstCodemsn = generatormsn.LoadLastCodemsn(lastCodeMSN);
                firstCodemsc = generatormsc.LoadLastCodemsc(lastCodeMSC);
            }
                        ...
                        if (var_op_type == "AL")
                        {
                            ...
                            switch (caseSwitch)
                            {
                                ...
                                case "FP":
                                    ...
                                    {
                                        my_data = F_Padding(newRow[variableID].ToString(), int.Parse(len_data));
                                    }
                                    break;
                                ...
                                case "ICCID_LD_U":
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            my_data = newRow[variableID].ToString();
                                            my_data += GetLuhnCheckDigit_U(newRow[variableID].ToString());
                                ...
                                case "SHA1":
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            my_data = ComputeHash(newRow[variableID].ToString(), "SHA1");
                                        }
                                    }
                                    break;
                                ...
                                case "SHA256":
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            my_data = ComputeHash(newRow[variableID].ToString(), "SHA256");
                                        }
                                    }
                                    break;
                                ...
                                case "SHA512":
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            my_data = ComputeHash(newRow[variableID].ToString(), "SHA512");
                                        }
                                    }
                                    break;
                                ...
                                case "euicc_cert_data":
                                    ...
                                    else
                                    {
                                        ...
                                        if (response_data.Contains("CKR_GENERAL_ERROR"))
                                        {
                                            logString.AppendLine($"**Error: HSM connectivity error");
            ...
            else
            {
                ...
                try
                {
                    ...
                    //updating last msn and msc generated for reliance in CodeTracker table
                    if (customer_name_form.ToUpper() == "RELIANCE")
                    {
                        using (SqlConnection con_reliance = new SqlConnection(connectionString))
                        {
                            con_reliance.Open();
                            // STEP 5 : Save last generated code
                            using (SqlCommand cmd = new SqlCommand(@"INSERT INTO CodeTracker 
                    (Prefix, LastGeneratedCodemsn,LastGeneratedCodemsc,FirstGeneratedCodeMSN,FirstGeneratedCodeMSC,InputFilename,TKkeyUsed,OPkeyUsed)
            VALUES (@prefix,@codemsn,@codemsc,@firstCodemsn,@firstCodemsc,@InputFilename,@TKkeyUsed,@OPkeyUsed)", con_reliance))
                            {
                                cmd.Parameters.AddWithValue("@prefix", prefix);
                                cmd.Parameters.AddWithValue("@codemsn", lastCodeMSN);
                                cmd.Parameters.AddWithValue("@codemsc", lastCodeMSC);
                                cmd.Parameters.AddWithValue("@firstCodemsn", firstCodemsn);
                                cmd.Parameters.AddWithValue("@firstCodemsc", firstCodemsc);
                                cmd.Parameters.AddWithValue("@InputFilename", Path.GetFileName(filename_2));
                                cmd.Parameters.AddWithValue("@TKkeyUsed", TKkeyUsed);
                                cmd.Parameters.AddWithValue("@OPkeyUsed", OPkeyUsed);
                                cmd.ExecuteNonQuery();
                            }
                        }
                    }
        ...
        private void ProcessAllFile(int customerProfileID)
        {
            ...
            try
            {
                ...
                foreach (var fileName in fileNames)
                {
                    ...
                    if (data_status == 0)
                    {
                       logString.Append($"**Error: Output generation failed for file: {fileName_ProcessAllFile}");
                        throw new InvalidOperationException("Output generation failed.");
                    }
                }
            }
            ...
            {
                logString.Append("**Error:Something went wrong with: " + fileName_ProcessAllFile + "\n" + ex.Message + "\n" + ex.StackTrace);
        ...
        public int generate_AllTypeOutput_new(string filetype)
        {
            try
            {
                string rootdir = "", outfileLocation = "", filenameconv = "", filemasterid = "";
                string fileext = "", custProfileFileID = "";
                int count = 0;
                string myfile = string.Empty;

                // 1. Fetch File Metadata Safely using Parameters
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    const string metaQuery = @"
                SELECT FilePath, FileNamingConv, FileMasterID, FileExtn, CustProfileFileID 
                FROM CustProfileFile 
                WHERE CustProfileID = @ProfileID 
                  AND CustomerID = @CustomerID 
                  AND FileName = @FileType 
                  AND FileIOID <> 'I'";

                    using (SqlCommand cmd = new SqlCommand(metaQuery, con))
                    {
                        cmd.Parameters.AddWithValue("@ProfileID", ProfileID);
                        cmd.Parameters.AddWithValue("@CustomerID", customerID);
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                rootdir = reader["FilePath"].ToString().TrimEnd();
                                filenameconv = reader["FileNamingConv"].ToString().TrimEnd();
                                filemasterid = reader["FileMasterID"].ToString().TrimEnd();
                                fileext = reader["FileExtn"].ToString().TrimEnd();
                                custProfileFileID = reader["CustProfileFileID"].ToString().TrimEnd();
                            }
                            else
                            {
                                logString.Append("**Error: Metadata profile file records not found.");
                                throw new Exception("Metadata profile file records not found.");
                            }
                        }
                    }
                }

                // Determine directory location path securely
                if (customer_name_form.Equals("RELIANCE", StringComparison.OrdinalIgnoreCase))
                {
                    outfileLocation = Path.Combine(rootdir, customer, profile, $"{generic_batch_no}_{FileProcessingLotID}");
                }
                else
                {
                    outfileLocation = Path.Combine(rootdir, customer, profile, $"{FileProcessingLotID}_{lastInsertedId}_{unixTime}");
                }
                ...
                // 2. Open Primary Decrypted Connection for Processing Data
                string constr = EncryptionandDecryption.DecryptString(ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString);
                using (SqlConnection con = new SqlConnection(constr))
                {
                    ...
                    string header = "";
                    using (SqlCommand cmd = new SqlCommand("SELECT Header FROM OutFileTemplateHD WHERE [ProfileFileID]=@MasterID AND [ProfileID]=@ProfileID", con))
                    {
                        cmd.Parameters.AddWithValue("@MasterID", filemasterid);
                        cmd.Parameters.AddWithValue("@ProfileID", ProfileID);
                        header = cmd.ExecuteScalar()?.ToString() ?? "";
                    }

                    // Fetch Footer Template
                    string footer = "";
                    using (SqlCommand cmd = new SqlCommand("SELECT Footer FROM OutFileTemplateFT WHERE [ProfileFileID]=@MasterID AND [ProfileID]=@ProfileID", con))
                    {
                        cmd.Parameters.AddWithValue("@MasterID", filemasterid);
                        cmd.Parameters.AddWithValue("@ProfileID", ProfileID);
                        footer = cmd.ExecuteScalar()?.ToString() ?? "";
                    }

                    // Cache First/Last IMSI & ICCID once upfront instead of querying iteratively inside loops
                    string firstImsi = "", lastImsi = "", firstIccid = "", lastIccid = "";

                    using (SqlCommand cmd = new SqlCommand($"SELECT TOP 1 V004, V005 FROM DataGenProcessDataRecord WHERE DataGenProcessHDID={lastInsertedId} ORDER BY DataGenProcessDataRecordID ASC", con))
                    using (var r = cmd.ExecuteReader()) if (r.Read()) { firstImsi = r[0].ToString(); firstIccid = r[1].ToString(); }

                    using (SqlCommand cmd = new SqlCommand($"SELECT TOP 1 V004, V005 FROM DataGenProcessDataRecord WHERE DataGenProcessHDID={lastInsertedId} ORDER BY DataGenProcessDataRecordID DESC", con))
                    using (var r = cmd.ExecuteReader()) if (r.Read()) { lastImsi = r[0].ToString(); lastIccid = r[1].ToString(); }

                    // Fetch DataGenProcessData Variables
                    DataTable dtVariables = new DataTable();
                    using (SqlCommand cmd = new SqlCommand("SELECT VarName, VarValue FROM [DataGenProcessData] WHERE DataGenProcessHDID=@HDID", con))
                    {
                        cmd.Parameters.AddWithValue("@HDID", lastInsertedId);
                        using (SqlDataReader reader = cmd.ExecuteReader()) dtVariables.Load(reader);
                    }

                    // FIX: Defined explicitly as a proper local Func lambda expression to pass compiler checks
                    Func<string, string> ParseTemplateTokens = (string textTemplate) =>
                    {
                        if (string.IsNullOrEmpty(textTemplate)) return string.Empty;

                        string pattern = @"\{([^{}]*)\}";
                        MatchCollection matches = Regex.Matches(textTemplate, pattern);

                        foreach (Match match in matches)
                        {
                            string varToken = match.Groups[1].Value.Trim();
                            string replacement = "";

                            switch (varToken.ToLower())
                            {
                                case "profile": replacement = profile.Trim(); break;
                                case "customer": replacement = customer.Trim(); break;
                                case "first_imsi": replacement = firstImsi; break;
                                case "last_imsi": replacement = lastImsi; break;
                                case "first_iccid": replacement = firstIccid; break;
                                case "last_iccid": replacement = lastIccid; break;
                                default:
                                    DataRow[] foundRows = dtVariables.Select($"VarName = '{varToken.Replace("'", "''")}'");
                                    replacement = foundRows.Length > 0 ? foundRows[0]["VarValue"].ToString().Trim() : "";
                                    break;
                            }
                            textTemplate = textTemplate.Replace("{" + varToken + "}", replacement);
                        }
                        return textTemplate;
                    };

                    header = ParseTemplateTokens(header);
                    footer = ParseTemplateTokens(footer);

                    // 3. Resolve Outbound Filename Structures
                    if (filenameconv.Trim() == "FROMFILE")
                    {
                        using (SqlCommand cmd = new SqlCommand("SELECT TOP 1 t2.FilePath FROM [CustProfileFile] t1 INNER JOIN [DataGenProcessHDFile] t2 ON t1.CustProfileFileID=t2.CustProfileFileID WHERE FileIOID='I' AND t2.DataGenProcessHDID=@HDID", con))
                        {
                            cmd.Parameters.AddWithValue("@HDID", lastInsertedId);
                            string matchedPath = cmd.ExecuteScalar()?.ToString() ?? "";
                            string cleanBaseName = Path.GetFileNameWithoutExtension(matchedPath).Trim();

                            if (customer_name_form.Equals("RELIANCE", StringComparison.OrdinalIgnoreCase) && filetype != "CPD")
                            {
                                outfileLocation = Path.Combine(outfileLocation, cleanBaseName.Replace("IN_", "OUT_"));
                                Directory.CreateDirectory(outfileLocation);
                            }
                            myfile = Path.Combine(outfileLocation, cleanBaseName + fileext);
                        }
                    }
                    else if (filenameconv.Trim().StartsWith("FROMFILE_"))
                    {
                        string[] parts = filenameconv.Split(new[] { '_' }, StringSplitOptions.RemoveEmptyEntries);
                        using (SqlCommand cmd = new SqlCommand("SELECT TOP 1 t2.FilePath FROM CustProfileFile t1 INNER JOIN DataGenProcessHDFile t2 ON t1.CustProfileFileID = t2.CustProfileFileID WHERE t1.FileIOID = 'I' AND t2.DataGenProcessHDID = @HDID", con))
                        {
                            cmd.Parameters.AddWithValue("@HDID", lastInsertedId);
                            string cleanBaseName = Path.GetFileNameWithoutExtension(cmd.ExecuteScalar()?.ToString() ?? "");
                            ...
                            string replacement = parts.Length > 1 ? string.Join("_", parts.Skip(1)) : "";
                            myfile = Path.Combine(outfileLocation, cleanBaseName.Replace("IN_", replacement + "_") + fileext);
                        }
                    }
                    else
                    {
                        myfile = Path.Combine(outfileLocation, $"{filenameconv}{unixTime}_{lastInsertedId}{fileext}");
                    }

                    // 4. Construct Content Query Line Rules Programmatically 
                    DataTable dtLines = new DataTable();
                    using (SqlCommand cmd = new SqlCommand("SELECT Varname, vartype FROM OutputTemplateLines WHERE ProfileFileID = @MasterID AND ProfileId = @ProfileID ORDER BY OutputTemplateLinesID", con))
                    {
                        cmd.Parameters.AddWithValue("@MasterID", filemasterid);
                        cmd.Parameters.AddWithValue("@ProfileID", ProfileID);
                        using (SqlDataReader reader = cmd.ExecuteReader()) dtLines.Load(reader);
                    }

                    StringBuilder dynamicColumns = new StringBuilder();
                    foreach (DataRow row in dtLines.Rows)
                    {
                        string varName = row["Varname"].ToString().TrimEnd();
                        string varType = row["vartype"].ToString();

                        if (dynamicColumns.Length > 0 && varType[0] == 'V') dynamicColumns.Append("+");

                        if (varType[0] == 'V')
                        {
                            dynamicColumns.Append(varName);
                        }
                        else if (varType == "T")
                        {
                            dynamicColumns.Append($"+'{varName}'");
                        ...
                        {
                            string sepChar = " ";
                            using (SqlCommand cmdSep = new SqlCommand("SELECT TRIM(Seperator) FROM [dbo].[SeperatorMaster] WHERE SepID = @SepID", con))
                            {
                                cmdSep.Parameters.AddWithValue("@SepID", varName);
                                sepChar = cmdSep.ExecuteScalar()?.ToString() ?? " ";
                            }
                            dynamicColumns.Append($"+'{sepChar}'");
                        }
                    }

                    // 5. Query Targeted Production Record Rows
                    string recordsQuery = $"SELECT {dynamicColumns} FROM DataGenProcessDataRecord WHERE DataGenProcessHDID = @HDID ORDER BY [DataGenProcessDataRecordID]";
                    DataTable dtRecords = new DataTable();
                    using (SqlCommand cmd = new SqlCommand(recordsQuery, con))
                    {
                        cmd.Parameters.AddWithValue("@HDID", lastInsertedId);
                        using (SqlDataReader reader = cmd.ExecuteReader()) dtRecords.Load(reader);
                    }

                    // 6. Output Processing Engine (IO Streaming Generation)
                    if (filetype == "XLSX") myfile = myfile.Replace("xlsx", "csv");

                    using (FileStream fs = new FileStream(myfile, FileMode.Create, FileAccess.Write, FileShare.None))
                    {
                        // FIX: Pass string path destination instead of raw stream SafeFileHandle resource pointers
                        ...
                        using (StreamWriter writer = new StreamWriter(fs))
                        {
                            ...
                            // Specialized format branch handlers
                            if (filetype == "CPD")
                            {
                                int cpdIndex = 0;
                                string blockValue11 = "", blockValue12 = "";

                                foreach (DataRow dr in dtRecords.Rows)
                                {
                                    string[] values = dr[0].ToString().Split(';');
                                    if (cpdIndex % 500 == 0) blockValue11 = values.ElementAtOrDefault(11) ?? "";
                                    else if (values.Length > 11 && values[11] != blockValue11) { values[11] = "FFFFFFFFFFFFFFFFFFFF"; values[16] = "FFFFFFFFFFFFFFFFFFFF"; }

                                    if ((cpdIndex + 1) % 500 == 0) blockValue12 = values.ElementAtOrDefault(12) ?? "";
                                    else if (values.Length > 12 && values[12] != blockValue12) values[12] = "FFFFFFFFFFFFFFFFFFFF";
                            ...
                            else if (filetype == "TXT" && customer_name_form.Equals("RELIANCE", StringComparison.OrdinalIgnoreCase))
                            {
                                if (dtRecords.Rows.Count > 0)
                                {
                                    string baseLine = dtRecords.Rows[0][0].ToString();
                                    writer.WriteLine(baseLine.Replace("IN_", "CNUM_") + ".txt");
                                    writer.WriteLine(baseLine.Replace("IN_", "SCM_") + ".txt");
                                    writer.WriteLine(baseLine.Replace("IN_", "SIMODA_") + ".cps");
                                }
                            }
                            else if (fileext.Equals(".mca", StringComparison.OrdinalIgnoreCase) &&
                                    (batchtypename.Equals("QUARTER", StringComparison.OrdinalIgnoreCase) || batchtypename.Equals("MFF2", StringComparison.OrdinalIgnoreCase)))
                            {
                                int headerColCount = string.IsNullOrWhiteSpace(header) ? 0 : header.TrimEnd().Split(',').Length;
                                foreach (DataRow dr in dtRecords.Rows)
                                {
                                    string[] values = dr[0].ToString().Split(',');
                                    bool isCP = values.Last().Trim().Equals("CP", StringComparison.OrdinalIgnoreCase);

                                    var firstPart = values.Take(headerColCount);
                                    var lastPart = isCP ? values.Skip(values.Length - 5) : values.Skip(values.Length - 4);

                                    writer.WriteLine(string.Join(",", firstPart.Concat(lastPart)));
                                    count++;
                                }
                            }
                            else
                    ...
                    // 7. Post-Processing Batch Management (.mca conditional grouping splits & AES encryption wrapping)
                    if (fileext.Equals(".mca", StringComparison.OrdinalIgnoreCase))
                    {
                        int mcaBatchSize = batchsize;
                        // FIX: Handled missing assignment logic causing internal string-to-int conversion mismatches 
                        batchsize = (batchsize == 0) ? 2500 : batchsize;

                        CreateMCABatch(myfile, batchsize, 500, Po_Num, customer_name_form);
                        Total_no_of_files += 1;

                        string[] lines = File.ReadAllLines(myfile);
                        if (IsSingle && (lines.Length - 1) > mcaBatchSize && mcaBatchSize > 0)
                        {
                            int numFiles = (int)Math.Ceiling((double)(lines.Length - 1) / mcaBatchSize);
                            ...
                            for (int i = 0; i < numFiles; i++)
                            {
                                ...
                                using (FileStream fs = new FileStream(outputFile, FileMode.Create, FileAccess.Write, FileShare.None))
                                {
                                    ...
                                    {
                                        writer.WriteLine(lines[0]);
                                        for (int j = 1; (j <= mcaBatchSize && (j + i * mcaBatchSize) <= lines.Length - 1); j++)
                                        {
                                            writer.WriteLine(lines[j + i * mcaBatchSize]);
                                        }
                                    }
                                }
                                ...
                                logString.Append($"    - Filename : {Path.GetFileName(outputFile)}\n");
                                logString.Append($"    - No of record : {mcaBatchSize}\n");
                        ...
                        {
                            string paddedFile = myfile.Replace(fileext, "_0001" + fileext);
                            if (File.Exists(paddedFile)) File.Delete(paddedFile);
                            File.Move(myfile, paddedFile);

                            string finalOut = (profilename == "EUICC") ? paddedFile : EncryptionandDecryption.AESEncrypt_File(paddedFile, OFProcessing.file_enc_key);
                            if (File.Exists(finalOut)) File.SetAttributes(finalOut, FileAttributes.Normal);

                            UpdateProcessHDFile($"{filetype}", Convert.ToInt32(custProfileFileID.Trim()), Path.GetFileName(finalOut), finalOut, lastInsertedId);
                        }
                    }
                    else
                    {
            ...
            {
                MessageBox.Show($"Something went wrong while creating outfile {filetype} for HDID {lastInsertedId}. Error: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                deletion_errorneous_data(lastInsertedId.ToString());
                logString.Append($"\nSomething went wrong while creating outfile {filetype} for HDID {lastInsertedId}. Error: {ex.Message}");
                return 0;
            }
        }
        public int generate_AllTypeOutput(string filetype)
        {
            try
            {
                ...
                string rootdir = "", Outfilelocation ="";
                string filenameconv = "";
                string filemasterid = "";
                string fileext = "";
                    ...
                    if (Header != "")
                    {
                        ...
                        if (matches.Count > 0)
                        {
                            foreach (Match match in matches)
                            {
                                ...
                                else if (var.ToLower() == "first_imsi")
                                {
                                    using (SqlConnection connection = new SqlConnection(connectionString))
                                    {
                                        SqlCommand command = new SqlCommand($"select Top(1)V004 from DataGenProcessDataRecord where DataGenProcessHDID={lastInsertedId}  order by [DataGenProcessDataRecordID]", connection);
                                        connection.Open();
                                        using (SqlDataReader reader = command.ExecuteReader())
                                        {
                                            if (reader.Read())
                                ...
                                else if (var.ToLower() == "last_iccid")
                                {
                                    using (SqlConnection connection = new SqlConnection(connectionString))
                                    {
                                        SqlCommand command = new SqlCommand($"select Top(1)V005 from DataGenProcessDataRecord where DataGenProcessHDID={lastInsertedId}  order by [DataGenProcessDataRecordID] desc", connection);
                                        connection.Open();
                                        using (SqlDataReader reader = command.ExecuteReader())
                                        {
                                            if (reader.Read())
                                ...
                                else if (var.ToLower() == "first_iccid")
                                {
                                    using (SqlConnection connection = new SqlConnection(connectionString))
                                    {
                                        SqlCommand command = new SqlCommand($"select Top(1)V005 from DataGenProcessDataRecord where DataGenProcessHDID={lastInsertedId}  order by [DataGenProcessDataRecordID]", connection);
                                        connection.Open();
                                        using (SqlDataReader reader = command.ExecuteReader())
                                        {
                                            if (reader.Read())
                    ...
                    if (filenameconv.Trim() == "FROMFILE")
                    {
                        using (SqlConnection connection = new SqlConnection(connectionString))
                        {
                            ...
                            using (SqlDataReader reader = command.ExecuteReader())
                            {
                                if (reader.Read())
                                {
                                    if (customer_name_form.ToUpper() == "RELIANCE" && filetype != "CPD")
                                    {
     ...
     batchtypename.Equals("MFF2", StringComparison.OrdinalIgnoreCase))))
                    ...
                    if (fileext.ToLower().Trim() == ".mca")
                    {
                        ...
                        string[] lines = File.ReadAllLines(myfile);
                        if (IsSingle && (lines.Length - 1) > mca_batchsize && mca_batchsize > 0)
                        {
        ...
        public static string CreateMCABatch(string mcaFile, int outerBatchSize, int innerBatchSize, string poNumber, string customerName)
        {
            ...
            if (!merged_batch_list.Contains(batch_list))
        ...
        public void MergeCsvsFromFolders(string[][] allFileSets)
        {
            if (allFileSets == null || allFileSets.Length == 0)
            {
                logString.Append("**Error: No file sets provided.**\n");
        ...
        public string F_Padding(string input, int length)
        {
            string padding = input.PadRight(length, 'F');
        ...
        public string GetLuhnCheckDigit_U(string number)
        {
            var sum = 0;
            var alt = true;
            var digits = number.ToCharArray();
        ...
        public static string ComputeHash(string input, string algorithm)
        {
            ...
            using (hashAlgorithm)
            {
                ...
                return BitConverter.ToString(hash).Replace("-", "").ToString().Substring(0, 6);
            }
        }
        ...
        private void cbxProfile_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbxProfile.SelectedIndex > 0)
            {
                ...
                if (batch != null && batch.Count > 0)
                {
                    ...
                    if (cbxProfile.Text.ToUpper().Contains("DUMMY"))
                    {
                        txtInputfile.Text = "Selected profile has no Input file";
                        btnInputFile.Enabled = false;
    ...
    public class BatchCodeGenerator
    {
        ...
        public string LoadLastCodemsn(string lastCode)
        {
            ...
            return $"{_lastPrefix}{_letter}{_number:D3}";
        }
        public string LoadLastCodemsc(string lastCode)
        {
            if (string.IsNullOrEmpty(lastCode))
                return "";
            ...
            return $"{_lastPrefix}M{_letter}{_number:D2}";
        }
